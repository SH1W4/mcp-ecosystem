...
class SessionManager:
    ...
    async def _validate_tools(self) -> bool:
        ...
        for tool, required_version in self.tool_requirements.items():
            try:
                ...
                is_valid = self._check_version(version, required_version) if required_version else True
                ...
                if is_valid:
                    self.logger.info(f"✅ {tool}: {version} (req: {required_version or 'any'})")
                else:
                ...
                self.session_report.issues_found.append(f"Tool check failed: {tool} - {e}")
                all_valid = False
    ...
    async def _fix_tool_issue(self, tool: str, current: str, required: str):
        ...
        if tool == "git" and current == "not_found":
            ...
            self.logger.warning("📝 Node.js não encontrado. Considere instalar via winget ou nvm")
    ...
    async def _check_package_files(self) -> bool:
        ...
        package_files = ["package.json", "Cargo.toml", "pyproject.toml", "requirements.txt"]
        found = False
    ...
    async def _record_action(self, action: str):
        ...
        self.session_report.actions_taken.append(f"{datetime.now().isoformat()}: {action}")
    ...
    def generate_report(self) -> Dict:
        ...
        return {
            ...
            "summary": {
                "duration": str(self.session_report.end_time - self.session_report.start_time) if self.session_report.end_time else "ongoing",
...
async def main():
    ...
    parser.add_argument("command", choices=["start", "end", "status"], help="Comando a executar")
